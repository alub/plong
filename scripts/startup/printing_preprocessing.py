...
class MeshVerificationPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.label(text="Active mesh is : %s" % mesh.name)
        ...
        # Third box of the panel (step 3) : Choice of the supporting plane
        box3 = layout.box()
...
class FindSupportingPlanesOperator(bpy.types.Operator):
    ...
    bl_description = "This function will find several possible planes on which your object can be printed. The first one to be displayed shall be the best one"
...
class VizualizeNextPlaneOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        """
        Allows to vizualize the object on the next supporting plane on the list.
        """
...
class VizualizePreviousPlaneOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        """
        Allows to vizualize the object on the previous supporting plane on the list.
        """
        global plane
...
class ChooseCurrentPlaneOperator(bpy.types.Operator):
    ...
    bl_label = "Choose the current plane"
...
def register():
    ...
    scn.FastProcessing = bpy.props.BoolProperty(name="Fast processing", description="Might not be as efficient as the normal processing", default=False)
